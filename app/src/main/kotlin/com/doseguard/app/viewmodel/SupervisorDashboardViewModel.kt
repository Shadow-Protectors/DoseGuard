...
class SupervisorDashboardViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val plantMetrics: StateFlow<PlantMetrics> = combine(
        ...
        val assignedBands = bands.filter { it.workerId.isNotBlank() && it.bandStatus == "ASSIGNED" }
    ...
    val rosterList: StateFlow<List<WorkerRosterItem>> = combine(
        ...
        when (filter) {
            ...
            RosterFilter.ACTIVE_BANDS -> filteredBySearch.filter { it.activeBand != null && it.activeBand.bandStatus == "ASSIGNED" }
